...
export function validatePatchPolicy(patch: Patch):void {
  if (!patch.canPatch){
    throw new Error("A safe patch cannot be generated.");
  }
  ...
  if (patch.files.length > MAX_FILES_CHANGED) {
    throw new Error(`Patch changes more than ${MAX_FILES_CHANGED} files.`);
  }

  for (const file of patch.files){
    if (isProtectedPath(file.path)) {
      throw new Error(`Protected file detected: ${file.path}`);
    }

    const diffLines = countDiffLines(file.diff);

    if (diffLines > MAX_DIFF_LINES) {
      throw new Error(`Patch for ${file.path} is too large.`);
    }
  }
}
